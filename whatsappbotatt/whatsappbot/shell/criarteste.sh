...
nome="tzap"$(echo $RANDOM | md5sum | head -c 5; echo;)
if [[ -z $nome ]]
then
exit 1
fi
...
limite=1
tempo=$(cat /etc/botvendas/tempo-test)
if [[ -z $tempo ]]
then
exit 1
fi
tuserdate=$(date "+%Y-%m-%d" -d "+2 days")
...
at -f /etc/SSHPlus/userteste/$nome.sh now + $tempo hour
[[ $tempo -gt 1 ]] && after="horas" || after="hora"
